...
def load_data(file_name):
    ...
    for i in xrange(m):
        if label_tmp[i] == kind[0]:
            ...
            label_data[i, 1] = 1
    # print feature_data
    temp = np.mat(feature_data)
    temp = Normaliziaton(temp)
    return temp, label_data, n_class

def sig(x):
    '''Sigmoid函数
    input:  x(mat/float):自变量，可以是矩阵或者是任意实数
    output: Sigmoid值(mat/float):Sigmoid函
数的值
    '''
...
if __name__ == "__main__":
    # 1、导入数据
    print "--------- 1.load data ------------"
    feature, label, n_class = load_data("train.csv")
    # 2、训练网络模型
    print "--------- 2.training ------------"
    w0, w1, b0, b1 = bp_train(feature, label, 20, 1000, 0.01, n_class)
    # 3、保存最终的模型
    print "--------- 3.save model ------------"
